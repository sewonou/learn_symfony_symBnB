...
class AdController extends AbstractController
{
     ...
     * @Security("is_granted('ROLE_USER') and user === ad.getAuthor()", message="Cette annonce ne vous appartient
     * pas, vous ne pouvez pas la modifier")
     * @return Response
     */
    public function edit(Ad $ad, Request $request, ObjectManager $manager)
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            $this->addFlash(
                'success',
                "Les modifications de l''annonce <strong> {$ad->getTitle()}</strong> ont été bien enregistrée"
            );
            return $this->redirectToRoute('ads_show', [
                'slug' => $ad->getSlug()
            ]);
